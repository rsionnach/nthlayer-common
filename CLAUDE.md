...
src/nthlayer_common/
    ...
    overrides/
        ...
        models.py        # OverrideEvent (gen_ai.override canonical form; required: decision_id/service/corrected_action/reviewer; tz-aware timestamp enforced; is_high_confidence_failure property; to_otel_attributes() drops None); OverridePrivacyConfig (plaintext_reviewer=False, exclude_reason=False); hash_reviewer() SHA-256 hex (GDPR pseudonymisation baseline, not anonymisation — reversible by enumeration); map_webhook_to_override() dotted-path adapter (coerces string confidence, rejects naive timestamps, surfaces required-field gaps as ValueError); HIGH_CONFIDENCE_THRESHOLD=0.85
        ingestion.py     # apply_override_to_verdict(store, event, *, privacy=None) -> Verdict | None — binds OverrideEvent to verdict store by decision_id; allowlist _OPEN_FOR_OVERRIDE={"pending"} (catches partial/confirmed/superseded/expired and unrecognised future statuses); idempotent on same-content re-apply (spec §4 scenario 11); returns None on unmatched ID / non-pending status / audit-trail conflict / TOCTOU race with concurrent delete (KeyError on update_outcome); first-write-wins semantic — event.timestamp recorded on Override but never compared; reviewer hashed by default
    manifest/
...
- `OverrideEvent` — required: `decision_id`, `service`, `corrected_action`, `reviewer`; optional: `original_action`, `reason`, `confidence_at_decision` (float 0.0–1.0), `source_system`, `timestamp` (tz-aware, default utcnow); `is_high_confidence_failure` property (confidence > 0.85); `to_otel_attributes()` → gen_ai.override.* attr dict (None-valued fields dropped)
- `OverridePrivacyConfig(plaintext_reviewer=False, exclude_reason=False)` — spec §4 GDPR baseline; SHA-256 hash is pseudonymisation not anonymisation (reversible by enumeration); operators needing stronger privacy should pre-hash with per-deployment HMAC
- `hash_reviewer(reviewer) -> str` — stable SHA-256 hex
- `map_webhook_to_override(payload, mapping, *, defaults=None) -> OverrideEvent` — generic dotted-path adapter for arbitrary webhook shapes (Jira, Slack, etc.); coerces string confidence; rejects naive timestamps; surfaces required-field gaps as ValueError at boundary
- `apply_override_to_verdict(store, event, *, privacy=None) -> Verdict | None` — binds OverrideEvent to verdict by decision_id; idempotent on same-content re-apply; returns None on unmatched ID / terminal status (confirmed/superseded/expired) / audit-trail conflict; logs warnings on all None-return paths
