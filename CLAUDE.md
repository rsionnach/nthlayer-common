...
- `apply_override_to_verdict(store, event, *, privacy=None) -> Verdict | None` — binds OverrideEvent to verdict by decision_id; idempotent on same-content re-apply; returns None on unmatched ID / terminal status (confirmed/superseded/expired/partial/any unknown future status) / audit-trail conflict; logs warnings on all None-return paths
